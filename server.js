...
// Uses the real Prometheus obfuscator (Lua) run through the wasmoon Lua VM,
// ported from the obfuscate-and-show web app. See prometheus-engine.js.
// ══════════════════════════════════════════════════════════════════════════════
const prometheus = require("./prometheus-engine");

// Map the dashboard's obfuscation levels to Prometheus presets.
const LEVEL_PRESETS = { light: "Weak", full: "Strong" };

async function PrometheusObf(source, level = "full") {
    const preset = LEVEL_PRESETS[level] || LEVEL_PRESETS.full;
    return prometheus.obfuscate(source, preset);
}

// Warm up the Lua VM at boot so the first upload isn't slow.
prometheus.preloadEngine().catch(e => console.error("[Prometheus] preload failed:", e.message));
...
app.post("/v1/projects/:project_id/script", apiLimiter, requireApiKey, async (req, res) => {
    ...
    let obfuscated;
    try { obfuscated = await PrometheusObf(source, level); }
